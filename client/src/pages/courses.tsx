...
import { FaShieldAlt, FaNetworkWired, FaJava } from 'react-icons/fa';
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-4">Learn New Skills</h1>
            ...
            filteredCourses.map((course: any) => (
              ...
              >
                ...
                <div className={`h-48 flex items-center justify-center ${
                  course.id.includes('python') ? 'bg-gradient-to-r from-yellow-400 to-blue-500' :
                  course.id.includes('javascript') ? 'bg-gradient-to-r from-yellow-400 to-yellow-600' :
                  course.id.includes('react') ? 'bg-gradient-to-r from-blue-400 to-cyan-500' :
                  course.id.includes('angular') ? 'bg-gradient-to-r from-red-500 to-red-700' :
                  course.id.includes('html') ? 'bg-gradient-to-r from-orange-500 to-red-500' :
                  course.id.includes('java') && !course.id.includes('javascript') ? 'bg-gradient-to-r from-red-600 to-orange-600' :
                  course.id.includes('node') ? 'bg-gradient-to-r from-green-500 to-green-700' :
                  course.id.includes('django') ? 'bg-gradient-to-r from-green-600 to-green-800' :
                  course.id.includes('selenium') ? 'bg-gradient-to-r from-green-400 to-blue-500' :
                  course.id.includes('jira') ? 'bg-gradient-to-r from-blue-600 to-blue-800' :
                  course.id.includes('cypress') ? 'bg-gradient-to-r from-gray-700 to-gray-900' :
                  course.id.includes('ethical-hacking') ? 'bg-gradient-to-r from-red-600 to-black' :
                  course.id.includes('network-security') ? 'bg-gradient-to-r from-blue-600 to-purple-700' :
                  course.id.includes('docker') ? 'bg-gradient-to-r from-blue-500 to-blue-700' :
                  course.id.includes('kubernetes') ? 'bg-gradient-to-r from-blue-600 to-purple-600' :
                  course.id.includes('aws') ? 'bg-gradient-to-r from-orange-400 to-yellow-400' :
                  course.id.includes('sap') ? 'bg-gradient-to-r from-blue-700 to-blue-900' :
                  'bg-gradient-to-r from-blue-500 to-purple-600'
                }`}>
                  {course.id.includes('python') && <SiPython className="w-16 h-16 text-white" />}
                  {course.id.includes('javascript') && <SiJavascript className="w-16 h-16 text-white" />}
                  {course.id.includes('react') && <SiReact className="w-16 h-16 text-white" />}
                  {course.id.includes('angular') && <SiAngular className="w-16 h-16 text-white" />}
                  {course.id.includes('html') && <SiHtml5 className="w-16 h-16 text-white" />}
                  {course.id.includes('java') && !course.id.includes('javascript') && <FaJava className="w-16 h-16 text-white" />}
                  {course.id.includes('node') && <SiNodedotjs className="w-16 h-16 text-white" />}
                  {course.id.includes('django') && <SiDjango className="w-16 h-16 text-white" />}
                  {course.id.includes('selenium') && <SiSelenium className="w-16 h-16 text-white" />}
                  {course.id.includes('jira') && <SiJira className="w-16 h-16 text-white" />}
                  {course.id.includes('cypress') && <SiCypress className="w-16 h-16 text-white" />}
                  {course.id.includes('ethical-hacking') && <FaShieldAlt className="w-16 h-16 text-white" />}
                  {course.id.includes('network-security') && <FaNetworkWired className="w-16 h-16 text-white" />}
                  {course.id.includes('docker') && <SiDocker className="w-16 h-16 text-white" />}
                  {course.id.includes('kubernetes') && <SiKubernetes className="w-16 h-16 text-white" />}
                  {course.id.includes('aws') && <SiAmazon className="w-16 h-16 text-white" />}
                  {course.id.includes('sap') && <SiSap className="w-16 h-16 text-white" />}
                  {![
                    'python', 'javascript', 'react', 'angular', 'html', 'java', 'node', 'django',
                    'selenium', 'jira', 'cypress', 'ethical-hacking', 'network-security', 'docker',
                    'kubernetes', 'aws', 'sap'
                  ].some(tech => course.id.includes(tech)) && <BookOpen className="w-16 h-16 text-white" />}
                </div>
        ...
        {selectedCategory === 'all' && (
          <div className="mt-16">
            <h2 className="text-2xl font-bold text-foreground mb-6">Most Popular Courses</h2>
